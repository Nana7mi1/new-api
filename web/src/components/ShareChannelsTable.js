...
import EditShareTagModal from '../pages/Share/EditShareTagModal.js';
...
const ChannelsTable = () => {
  ...
  const loadChannels = async (startIdx, pageSize, idSort, enableTagMode) => {
    ...
    const res = await API.get(
      `/api/share/?p=${startIdx}&page_size=${pageSize}&id_sort=${idSort}&tag_mode=${enableTagMode}`
    );
    if (res === undefined) {
      return;
    }
  ...
  const copySelectedChannel = async (record) => {
    ...
    try {
      ...
      const response = await API.post('/api/share/', newChannel);
  ...
  const manageChannel = async (id, action, record, value) => {
    ...
    switch (action) {
      case 'delete':
        res = await API.delete(`/api/share/${id}/`);
        break;
      case 'enable':
        data.status = 1;
        res = await API.put('/api/share/', data);
        break;
      case 'disable':
        data.status = 2;
        res = await API.put('/api/share/', data);
  ...
  const manageTag = async (tag, action) => {
    ...
    switch (action) {
      case 'enable':
        res = await API.post('/api/share/tag/enabled', {
          tag: tag
        });
        break;
      case 'disable':
        res = await API.post('/api/share/tag/disabled', {
          tag: tag
        });
        break;
    }
  ...
  const searchChannels = async (searchKeyword, searchGroup, searchModel, enableTagMode) => {
    ...
    setSearching(true);
    const res = await API.get(
      `/api/share/search?keyword=${searchKeyword}&group=${searchGroup}&model=${searchModel}&id_sort=${idSort}&tag_mode=${enableTagMode}`
    );
    const { success, message, data } = res.data;
    if (success) {
      setChannelFormat(data, enableTagMode);
  ...
  const testChannel = async (record, model) => {
    const res = await API.get(`/api/share/test/${record.id}?model=${model}`);
  ...
  const updateChannelBalance = async (record) => {
    const res = await API.get(`/api/share/update_balance/${record.id}/`);
  ...
  const testAllChannels = async () => {
    const res = await API.get(`/api/share/test`);
    const { success, message } = res.data;
    if (success) {
  ...
  const deleteAllDisabledChannels = async () => {
    const res = await API.delete(`/api/share/disabled`);
    const { success, message, data } = res.data;
    if (success) {
  ...
  const updateAllChannelsBalance = async () => {
    ...
    const res = await API.get(`/api/share/update_balance`);
    const { success, message } = res.data;
    if (success) {
  ...
  const batchDeleteChannels = async () => {
    ...
    const res = await API.post(`/api/share/batch`, { ids: ids });
    const { success, message, data } = res.data;
    if (success) {
  ...
  const fixChannelsAbilities = async () => {
    const res = await API.post(`/api/share/fix`);
    const { success, message, data } = res.data;
    if (success) {
  ...
  const submitTagEdit = async (type, data) => {
    ...
    try {
      const res = await API.put('/api/share/tag', data);
      if (res?.data?.success) {
        showSuccess('更新成功！');
        await refresh();
      }
  ...
  const batchSetChannelTag = async () => {
    ...
    const res = await API.post('/api/share/batch/tag', {
      ids: ids,
