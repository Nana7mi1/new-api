...
const EditChannel = (props) => {
  ...
  const loadChannel = async () => {
    setLoading(true);
    let res = await API.get(`/api/share/${channelId}`);
    if (res === undefined) {
      return;
    }
  ...
  const fetchUpstreamModelList = async (name) => {
    ...
    if (isEdit) {
      ...
      const res = await API.get('/api/share/fetch_models/' + channelId);
      ...
      if (!inputs?.['key']) {
        ...
        try {
          const res = await API.post('/api/share/fetch_models', {
  ...
  const fetchModels = async () => {
    try {
      let res = await API.get(`/api/share/models`);
  ...
  const submit = async () => {
    ...
    if (isEdit) {
      res = await API.put(`/api/share/`, {
        ...localInputs,
        id: parseInt(channelId)
      });
    } else {
      res = await API.post(`/api/share/`, localInputs);
    }
